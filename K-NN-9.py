...
def entropy(x, y):
    # 计算向量x和y的信息熵距离
    eps = 1e-8  # 防止log(0)的情况，添加一个小的epsilon
    entropy_xy = -np.sum(x * np.log(y + eps))  # 使用信息熵公式计算距离
    return entropy_xy
...
def classify(inX, dataSet, labels, k):
    ...
    # 计算每个数据点和输入向量之间的距离（这里使用信息熵作为距离）
    distances = [entropy(inX, dataSet[i]) for i in range(num)]
